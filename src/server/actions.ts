...
export async function saveManualTrade(id: number | null, formData: FormData): Promise<ActionResult> {
  return guard(async () => {
    ...
    // side yet, so it is costed at half.
    const [account] = await db
      .select({ rate: accounts.commissionPerContract })
      .from(accounts)
      .where(eq(accounts.id, values.accountId))
      .limit(1)
    const roundTurn = (account?.rate ?? 0) * values.qty
    const commission = exitAt ? roundTurn : roundTurn / 2
    ...
    const payload: typeof trades.$inferInsert = {
      ...
      grossPnl: values.netPnl + commission,
      commission,
      fees: 0,
    ...
    if (id) await db.update(trades).set(payload).where(eq(trades.id, id))
    else await db.insert(trades).values(payload)

    await rollupDailyStats(values.accountId)
    revalidateAll()
    return id ? 'Trade updated.' : 'Trade saved.'
  })
}
