...
- **A firm is a name and a website.** Rules vary *within* a firm (MFFU pays
  80% on some accounts, 90% on others; Apex sells one size with intraday or
  EOD drawdown), so profit split, payout policy and every limit live on the
  plan and on the account. `src/lib/propfirm/catalogue.ts` is generated by
  `scripts/import-catalogue.mts` — never edit it by hand.
